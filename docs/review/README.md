...
1. **Resolved for v1: Sensitive display metadata.** State and resource-change masks now apply before extracting display fields. Regression tests cover nested conditions, whole-value masks, prior-state replacement, malformed masks, source immutability, and endpoint-level leaks. Raw IDs remain internal to dependency resolution.
...
3. **Resolved for v1: Rebuild scheduling and slow-client delivery.** Initial and edited plans share one worker per workspace and a global concurrency limit. Buffered requests coalesce bursts, and slow SSE clients retain the newest snapshot. Race-tested scheduler and delivery regressions cover these boundaries; filesystem-event stress tests remain useful follow-up work.
