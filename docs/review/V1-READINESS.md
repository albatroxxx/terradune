...
This release packages the existing Resource Map, Plan, and Graph interface for Go installation. It also closes two earlier review findings: sensitivity-aware display metadata and serialized/coalesced planning with newest-snapshot delivery. It adds HTTP defense-in-depth, clean cancellation, one-shot failure status, a static GitHub Pages site, security policy, release automation, and launch drafts.
...
- Site navigation and copy command feedback were verified in Chromium. Home and docs have no page overflow at 320px and 390px; desktop metadata/assets load correctly. The site checker validates four HTML pages, local links/anchors, canonical/social metadata, image alternatives, JSON-LD version, and sitemap coverage.

Historical CI and publication results are recorded in the PR and release workflow. Current installation and validation steps are in the [release runbook](../RELEASING.md).
...
Historical launch-site captures have been retired. See the [current site](https://albatroxxx.github.io/terradune/) for Go installation.
