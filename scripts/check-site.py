#!/usr/bin/env python3
"""Validate the static Pages site without a frontend build dependency."""
...
SITE = ROOT / "site"
BASE = "https://albatroxxx.github.io/terradune/"
...
assert len(pages) == 4, "Update the expected site page count when adding a page"
titles = set()
canonicals = set()
for path, page in pages.items():
    ...
    for link in page.links + [page.meta.get("og:image", "")]:
        ...
        target = SITE / unquote(parsed.path.removeprefix("/terradune/"))
        if target.is_dir():
            target /= "index.html"
